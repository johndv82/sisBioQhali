    ...
    <div class="row form-group">
        <div class="col-md-12">
            <div class="table-responsive table--no-card m-b-30">
                <table id="tblProveedores" class="table table-sm table-striped table-bordered display">
                    <thead>
                        <tr>
                            <th>Nombre</th>
                            <th>Contacto</th>
                    ...
                    <tbody>
                        <tr v-for="item in datos_a_pintar" :key="item.id_prov">
                            <td>{{ item.nombre_prov }}</td>
                            <td>{{ item.contacto_prov }}</td>
                            <td>{{ item.codigo_prov }}</td>
                            <td>{{ item.ruc_prov }}</td>
                            <td>
                                <button v-bind:id="'btnEditar-' + item.id_prov" class="btn btn-sm btn-warning"
                                    @click="editar(item.id_prov)">Editar</button>&nbsp;
                                <button v-bind:id="'btnEliminar-' + item.id_prov" class="btn btn-sm btn-danger"
                                    @click="eliminar(item.id_prov)">Eliminar</button>
                            </td>
                        </tr>
                    </tbody>
                </table>
...
export default {
    ...
    computed: {
        ...
        datos_a_pintar() {
            this.datos_parciales = this.datos.filter(prov => prov.nombre_prov.toLowerCase().includes(this.busqueda.toLowerCase()));
